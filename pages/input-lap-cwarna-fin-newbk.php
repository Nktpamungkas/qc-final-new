<script type="text/javascript">
    function proses_nokk(){
        ...
        if (nokk == 0) {
            ...
            window.location.href='CWarnaFinNew&'+nokk;
        }
    }
    ...
    function proses_shift() {
        ...
        if (nokk == "") {
            ...
            window.location.href='CWarnaFinNew&'+nokk+'&'+shift;
        }
    }
</script>
<?php
...
{
	if (trim($_POST['nokk'])=="") {
		$pesanError[] = "<b>No Kartu Kerja</b> belum diisi,  harus Anda isi !";		
	}
	if (trim($_POST['shift'])=="") {
		$pesanError[] = "<b>Shift</b> belum dipilih,  harus Anda pilih !";		
	}
	...
	if (count($pesanError)>=1 ){
		...
		echo "<img src='../images/attention.png'> <br><hr>";
			...
			$noPesan++;
				echo "&nbsp;&nbsp; <font size=+0.5  color=#FF0000>$noPesan. $pesan_tampil</font><br>";	
			} 
		echo "</div> <br>"; 
	}
	else {
		$ceksql=mysql_query("SELECT * FROM `tbl_lap_inspeksi` WHERE `nokk`='$_GET[nokk]' and `shift`='$_POST[shift]' AND `pisah_kain`='$_POST[no_revisi]' AND DATE_FORMAT(tgl_update, '%Y-%m-%d') = DATE_FORMAT(now(), '%Y-%m-%d') AND `dept`='QCF' LIMIT 1");
    $cek=mysql_num_rows($ceksql);
	if($cek>0){
	...
	$sql1=mysql_query("UPDATE`tbl_lap_inspeksi` SET
	`no_order`='$order',
	`no_po`='$po',
	`pelanggan`='$pelanggan',
	`jenis_kain`='$jns',
	...
	if($sql1){
        ...
        echo "<script>swal({
            ...
            if (result.value) {
                window.location.href='CWarnaFinNew-$_POST[nokk]';
	...
	$warna=str_replace("'","''",$_POST['warna']);
	$catatan=str_replace("'","''",$_POST['catatan']);
	$sql=mysql_query("INSERT INTO `tbl_lap_inspeksi` SET
	`nokk`='$_POST[nokk]',
	`no_order`='$order',
	`no_po`='$po',
	`pelanggan`='$pelanggan',
...
//Data sudah disimpan di database mysql
$msql=mysql_query("SELECT * FROM `tbl_lap_inspeksi` WHERE `nokk`='$nokk' and `shift`='$_GET[shift]' AND DATE_FORMAT(tgl_update, '%Y-%m-%d') = DATE_FORMAT(now(), '%Y-%m-%d') AND `dept`='QCF' LIMIT 1");
$row=mysql_fetch_array($msql);
$crow=mysql_num_rows($msql);

//Data sudah disimpan di database mysql
$qryfin=mysql_query("SELECT * FROM `tbl_lap_inspeksi` WHERE `nokk`='$nokk' AND `dept`='QCF' ORDER BY id DESC LIMIT 1");
$rfin=mysql_fetch_array($qryfin);
$cekfin=mysql_num_rows($qryfin);

//Data belum disimpan di database mysql
$sql=mssql_query($conn,"select top 1
        ...
        <div class="col-md-6">
            ...
            <div class="form-group">
				<label for="shift" class="col-sm-3 control-label">Shift</label>
                    <div class="col-sm-2">
                        <select class="form-control chosen-select" name="shift" id="shift" onchange="proses_shift()">
