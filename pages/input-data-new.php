...
WHERE
    ...
    $stmt0   = db2_exec($conn1, $sqlDB20, ['cursor' => DB2_SCROLLABLE]);
    $rowdb20 = db2_fetch_assoc($stmt0);
...
GROUP BY
	STOCKTRANSACTION.ORDERCODE";
    $stmt1 = db2_exec($conn1, $sqlroll, ['cursor' => DB2_SCROLLABLE]);
    ...
    GROUP BY
        PRODUCTIONRESERVATION.ORDERCODE,
        PRODUCTIONRESERVATION.ITEMTYPEAFICODE";
    $stmtkg1 = db2_exec($conn1, $sqlkg, ['cursor' => DB2_SCROLLABLE]);
...
";
    $stmtwarna1 = db2_exec($conn1, $sqlwarna, ['cursor' => DB2_SCROLLABLE]);
	    ...
	    <div class="form-group">
            <label for="no_po" class="col-sm-3 control-label">PO</label>
                <div class="col-sm-5">
                    ...
                    value="<?php if ($cek > 0) {echo $rcek['no_po'];} else {echo $rowdb20['PO_NUMBER'];}?>" placeholder="PO" readonly>
                </div>
			</div>
	    <div class="form-group">
            <label for="pelanggan" class="col-sm-3 control-label">LANGGANAN</label>
                <div class="col-sm-5">
                    <input name="pelaggan" type="text" class="form-control" id="pelaggan"
                    value="<?php if ($cek > 0) {echo $rcek['pelanggan'];} else {echo $rowdb2['LANGGANAN'] . "/" . $rowdb20['BUYER'];}?>" placeholder="langganan" readonly>
                </div>
			</div>
		<div class="form-group">
	  ...
	  <div class="form-group">
		...
		<?php
            ...
            if ($cek > 0) {
                ...
                $tgl_pack  = new DateTime($rcek['tgl_pack']);
            } else {
                $tgl_warna = new DateTime($tgl_w);
                $tgl_pack  = new DateTime($sPack['tgl_mulai']);
            }
        $delay = $tgl_warna->diff($tgl_pack);
        if ($delay->days >= 3) {?> <span class='badge bg-red blink_me'><?php echo "Delay ";
    echo $delay->days;
    echo " Hari"; ?></span><?php }
?></font>
      </div>
   ...
   <?php }?>
		  ...
		  <input type="hidden" value="<?php if ($cek > 0) {
                                          ...
                                          echo $rowdb2['LANGGANAN'] . "/" . $rowdb20['BUYER'];
                                      }?>" name="pelanggan">
		  <input type="hidden" value="<?php if ($cek > 0) {
                                              echo $rcek['no_ko'];
                                          } else {
...
<?php
    if ($_POST['send'] == "send") {
        ...
        $mail->Password   = 'Xr7PzUWoyPA';             //fariz001 / D1t2017
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
        $mail->Port       = 587;

        $mail->setFrom('dept.it@indotaichen.com', 'DEPT IT');
        $mail->addReplyTo('deden.kurnia@indotaichen.com', 'ADM QCF');
        ...
        if ($_POST['ckqc'] == "1") {
            ...
            $mail->addCC('tobias.sulistiyo@indotaichen.com');
...
foreach ($listmail as $det) {
    // Menambahkan email ke daftar email (dipisahkan dengan koma)
    if ($emailList) {
        $emailList .= ',' . $det['email'];  // Jika sudah ada email sebelumnya, tambahkan koma
    } else {
        $emailList .= $det['email'];  // Jika pertama kali, hanya masukkan email pertama
    }

    // Ambil sales_detail pertama saja
    if ($salesDetail === '') {
        $salesDetail = $det['sales_detail'];  // Menyimpan nilai sales_detail hanya sekali
    }
}
