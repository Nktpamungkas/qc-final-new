...
<div class="box collapsed-box">
  <div class="box-header with-border">
    <h3 class="box-title"> Filter Laporan NSP</h3>
  ...
  <form method="post" enctype="multipart/form-data" name="form1" class="form-horizontal" id="form1">
      ...
      <div class="form-group">
		<div class="col-sm-2">
          ...
          <?php
                ...
                for ($y = 12; $y >= 1; $y--) {
                ?>
          <option value="<?php echo $y ?>" <?php if($Bln!=""){if($y==$Bln){echo "SELECTED";}}else{if($y==$bln_skr){echo "SELECTED";}}?>><?php echo $nmBln[$y]; ?></option>
          <?php
                }
   ?>
			</select>
   ...
   ?>
	...
	if(strlen($Bln)==1){$blnn="0".$Bln;}else{$blnn=$Bln;}
	$qry1=mysqli_query($con,"SELECT * FROM tbl_nsp_qcf WHERE $Wdept DATE_FORMAT( tgl_buat, '%Y-%m' )='$Thn-$blnn' $sts ORDER BY id ASC");
	$qrySUM=mysqli_query($con,"SELECT COUNT(*) as Lot, SUM(rol) as Rol,SUM(berat) as Berat FROM tbl_nsp_qcf WHERE $Wdept DATE_FORMAT( tgl_buat, '%Y-%m' )='$Thn-$Bln' $sts ");
...
<div class="row">
  <div class="col-xs-12">
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">Data NSP <?php echo $Dept ?></h3>
		<?php if($Bln!="" and $Thn!="") { ?> 
		<div class="pull-right">
		  <a href="pages/cetak/cetak_nsp.php?&bln=<?php echo $Bln; ?>&thn=<?php echo $Thn; ?>&dept=<?php echo $Dept; ?>&cancel=<?php echo $Cancel; ?>" class="btn btn-danger " target="_blank" data-toggle="tooltip" data-html="true" title="Laporan NCP Memo"><i class="fa fa-print"></i> Cetak</a>			
		</div>  
		<?php } ?>
		<?php if($Bln!="" and $Thn) { ?><br><b>Periode: <?php echo $nmBln[$Bln]." ".$Thn; ?></b>
	...
	while($row1=mysqli_fetch_array($qry1)){
		 ?>
          <tr bgcolor="<?php echo $bgcolor; ?>">
            ...
            <td align="center"><?php echo $row1['tgl_buat'];?><br><div class="btn-group"><a href="pages/cetak/bon_nsp.php?nokk=<?php echo $row1['nokk'];?>" class="btn btn-xs btn-danger" target="_blank"><i class="fa fa-print"></i></a><a href="pages/cetak/bon_ncp_pdf.php?nokk=<?php echo $row1['nokk'];?>" class="btn btn-xs btn-info" target="_blank"><i class="fa fa-file-pdf-o"></i></a></div></td>
            ...
            <td align="center"><?php echo $row1['no_order'];?><br><a href="Penyelesaian-<?php echo $row1['id']; ?>" class="btn <?php if(strtoupper($_SESSION['usrid'])!="ARIF"){ echo "disabled";}?>"></td>
