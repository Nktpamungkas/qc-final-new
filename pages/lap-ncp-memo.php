...
<div class="box box-warning collapsed-box">
  <div class="box-header with-border">
    <h3 class="box-title"> Filter Laporan NCP Memo</h3>
  ...
  <form method="post" enctype="multipart/form-data" name="form1" class="form-horizontal" id="form1">
	...
	$qry1=mysqli_query($con,"SELECT * FROM tbl_memo_qcf WHERE $Wdept DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts ORDER BY id ASC");
	$qrySUM=mysqli_query($con,"SELECT COUNT(*) as Lot, SUM(rol) as Rol,SUM(berat) as Berat FROM tbl_memo_qcf WHERE $Wdept DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts ");
...
<div class="row">
  <div class="col-xs-12">
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">Data NCP Memo <?php echo $Dept ?></h3>
		<?php if($Awal!="") { ?> 
		<div class="pull-right">
		  <a href="pages/cetak/cetak_ncpmemo.php?&awal=<?php echo $Awal; ?>&akhir=<?php echo $Akhir; ?>&dept=<?php echo $Dept; ?>&cancel=<?php echo $Cancel; ?>" class="btn btn-danger " target="_blank" data-toggle="tooltip" data-html="true" title="Laporan NCP Memo"><i class="fa fa-print"></i> Cetak</a>			
		</div>  
		<?php } ?>
	...
	while($row1=mysqli_fetch_array($qry1)){
		 ?>
          <tr bgcolor="<?php echo $bgcolor; ?>">
            ...
            <td><a href="#" class="posisi_kk" id="<?php echo $row1['nokk']; ?>">'<?php echo $row1['nokk'];?></a></td>
            <td><a href="#" class="btn sts_edit <?php if($_SESSION['dept']!="QC" or strtoupper($_SESSION['usrid'])!="ARIF"){ echo "disabled";}?>" id="<?php echo $row1['id']; ?>"><span class="label <?php if($row1['status']=="OK"){echo "label-success";}else if($row1['status']=="Cancel"){echo "label-danger";}else{echo "label-warning";} ?> "><?php echo $row1['status'];?></span></a></td>
