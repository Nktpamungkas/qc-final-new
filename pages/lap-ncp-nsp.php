...
<div class="box collapsed-box">
  <div class="box-header with-border">
    <h3 class="box-title"> Filter Laporan NCP/NSP</h3>
  ...
  <form method="post" enctype="multipart/form-data" name="form1" class="form-horizontal" id="form1">
	...
	$qry1=mysqli_query($con,"SELECT * FROM tbl_ncp_qcf WHERE $Wdept NOT nsp='' AND DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts ORDER BY id ASC");
	$qrySUM=mysqli_query($con,"SELECT COUNT(*) as Lot, SUM(rol) as Rol,SUM(berat) as Berat FROM tbl_ncp_qcf WHERE $Wdept NOT nsp=''AND DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts ");
...
<div class="row">
  <div class="col-xs-12">
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">Data NCP/NSP <?php echo $Dept ?></h3>
		<?php if($Awal!="") { ?> 
		<div class="pull-right">
		  <a href="pages/cetak/cetak_ncp_nsp.php?&awal=<?php echo $Awal; ?>&akhir=<?php echo $Akhir; ?>&dept=<?php echo $Dept; ?>&cancel=<?php echo $Cancel; ?>" class="btn btn-danger " target="_blank" data-toggle="tooltip" data-html="true" title="Laporan NCP"><i class="fa fa-print"></i> Cetak</a>			
		</div>  
		<?php } ?>
	...
	while($row1=mysqli_fetch_array($qry1)){
		 ?>
          <tr bgcolor="<?php echo $bgcolor; ?>">
            ...
            <td><?php if($row1['nsp2']!=""){echo $row1['nsp']."/".$row1['nsp1']."/".$row1['nsp2'];}if($row1['nsp1']!=""){echo $row1['nsp']."/".$row1['nsp1'];}else{echo $row1['nsp'];}?></td>
            </tr>
          <?php	$no++;  } ?>
        </tbody>
      </table>
