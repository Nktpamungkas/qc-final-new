            ...
            <form class="form-horizontal" name="modal_popup" data-toggle="validator" method="post" action="" enctype="multipart/form-data">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        ...
                        <h4 class="modal-title">Standard Color Migration</h4>
                </div>
                <div class="modal-body">
                        <div class="form-group">
                            <label for="hasil_test" class="col-md-2 control-label">Hasil Test</label>
                                <div class="col-md-8">
                                    <textarea name="hasil_test" class="form-control" placeholder="" rows="3"><?php if($r['cm_dye_temp']!="" or $rcekR['rcm_dye_temp']!=""){?>Suhu= <?php if($r['stat_cm']=="RANDOM"){echo $rcekR['rcm_dye_temp'];}else if($r['stat_cm']=="DISPOSISI"){echo $rcekD['dcm_dye_temp'];}else{echo $r['cm_dye_temp'];}?>&deg;C<?php } ?>
                                    <?php if($r['wash_colorchange']!="" or $rcekR['rwash_colorchange']!=""){?>&#13;&#10;Color Change= <?php if($r['stat_cm']=="RANDOM"){echo $rcekR['rcm_dye_colorchange'];}else if($r['stat_cm']=="DISPOSISI"){echo $rcekD['dcm_dye_temp'];}else{echo $r['cm_dye_colorchange'];}?><?php } ?>
                                    <?php if($r['cm_dye_stainingface']!="" or $rcekR['rcm_dye_stainingface']!=""){?>&#13;&#10;Staining= <?php if($r['stat_cm']=="RANDOM"){echo $rcekR['rcm_dye_stainingface'];}else if($r['stat_cm']=="DISPOSISI"){echo $rcekD['dcm_dye_stainingface'];}else{echo $r['cm_dye_stainingface'];}?><?php } ?>
                                    </textarea>
                                </div>
                        </div>
                        <div class="form-group">
                            <label for="cm" class="col-md-2 control-label">Standard</label>
                                <div class="col-md-8">
                                    <textarea name="cm" class="form-control" placeholder="">4-5 Color Change &#13;&#10;4 Color Staining</textarea>
                                </div>
                        </div>
		        </div>
