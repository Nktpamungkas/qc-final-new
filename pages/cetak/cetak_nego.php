...
<table width="100%">
  ...
  if($Awal!="" or $GShift!="" or $Subdept!="" or $Nego1=="1" or $Langganan!=""){
    $qry1=mysqli_query($con,"SELECT * FROM tbl_aftersales_now WHERE langganan LIKE '%$_GET[langganan]%' AND checknego!='' $Where $nego $shft $subdpt ORDER BY pejabat,personil ASC");
    }else{
    $qry1=mysqli_query($con,"SELECT * FROM tbl_aftersales_now WHERE langganan LIKE '$_GET[langganan]'  AND checknego!='' $Where $nego $shft $subdpt ORDER BY pejabat,personil ASC");
    }
  $tkirim=0;
        ...
        <tr>
          <td style="border-top:0px #000000 solid; 
            ...
            <tr>
              ...
              $tot_a=0;
              while($row1=mysqli_fetch_array($qry1)){
                $qryTotal = mysqli_query($con,"SELECT COUNT(checknego) AS total_lot_nego FROM tbl_aftersales_now WHERE checknego!='' 
                AND DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND nama_nego!='' AND sts_nego='1' $lgn");
                $rTotal=mysqli_fetch_array($qryTotal);
                $qryBN = mysqli_query($con,"SELECT COUNT(checknego) AS total_berhasil_nego FROM tbl_aftersales_now WHERE checknego='Ceklis' 
                ...
                $qryGN = mysqli_query($con,"SELECT COUNT(checknego) AS total_gagal_nego FROM tbl_aftersales_now WHERE checknego='Silang' 
                AND DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND nama_nego='$row1[nama_nego]' AND sts_nego='1' $lgn");
