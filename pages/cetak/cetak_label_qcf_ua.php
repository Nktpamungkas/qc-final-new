...
$dye=mysqli_query($condye,"SELECT * FROM tbl_schedule WHERE nokk='$r1[nokk_legacy]' and `status`='selesai' ORDER BY id DESC LIMIT 1");
$rdye=mysqli_fetch_array($dye);
?>
...
<table width="100%" border="0"style="width: 7in;">
  <tbody>    
	...
	border-left:0px #000000 solid; border-right:0px #000000 solid;"><span style="font-size:9px;"><?php echo date('d-M-Y');?> | Qty Yard : <?php if($cekr>0){echo $r['panjang'];}else{echo $r1['qty_yard'];}?> | Season : <?php if($cekr>0){echo $r['season'];}else{echo $r1['season'];}?></span></td>
          ...
          <tr>
          <td colspan="3" style="border-top:0px #000000 solid; border-bottom:0px #000000 solid;
	border-left:0px #000000 solid; border-right:0px #000000 solid;"><span style="font-size:9px;"><?php if($cekr>0){echo $r['availability'];}else{echo $r1['availability'];}?> | Mesin Dye : <?php if($rdye['no_mesin']!=''){echo $rdye['no_mesin'];}?></span></td>
        </tr>
      </table></td>
      <td align="left" valign="top" style="height: 1.6in;"><table width="100%" border="0" class="table-list1" style="width: 2.3in;">
        <tr>
