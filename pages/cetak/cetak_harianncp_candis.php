...
<table width="100%" border="0">
  ...
  if($Candis=="1"){ $sts_candis=" AND (`status`='Cancel' OR `status`='Disposisi') "; }else{ $sts_candis=" ";}		
	$qry1=mysqli_query($con,"SELECT * FROM tbl_ncp_qcf WHERE $Wdept DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts_dis $sts_cancel $sts_candis ORDER BY id ASC");
